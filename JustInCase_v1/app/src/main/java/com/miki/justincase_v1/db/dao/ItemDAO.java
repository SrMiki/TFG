...
public interface ItemDAO {
    ...
    @Query("SELECT * FROM items WHERE itemID IN " +
            "(SELECT categoryContent.FKitemID from categoryContent" +
            " WHERE FKcategoryID IS :categoryID) ORDER BY itemName")
    ...
    @Query("SELECT * FROM items WHERE itemID IN " +
            "(SELECT categoryContent.FKitemID from categoryContent" +
            " WHERE FKcategoryID IS :categoryID)")
    ...
    @Query("SELECT * FROM items WHERE itemID IN " +
            "(SELECT Baggage.FKitemID from Baggage " +
            "WHERE Baggage.FKHandLuggageID IS :handLuggageID) " +
            "ORDER BY itemName")
